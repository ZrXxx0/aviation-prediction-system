...
def process_and_reconcile_route(route, domestic, base_config):
    ...
    try:
        ...
        if result_m is None or result_q is None:
            ...
            return False
        # 存成 CSV 文件便于检查
        result_m.to_csv("check_result_monthly_tail.csv", index=False)
        result_q.to_csv("check_result_quarterly_tail.csv", index=False)

        print("\n✅ 已保存为 'check_result_monthly_tail.csv' 和 'check_result_quarterly_tail.csv'")
        # === 对齐预测 ===
        reconciled = mint_reconcile_monthly_to_quarterly(result_m, result_q)
